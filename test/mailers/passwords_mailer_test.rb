...
class PasswordsMailerTest < ActionMailer::TestCase
  test "the invitation renders, naming the site and the access being given" do
    body = Nibble::PasswordsMailer.invite(users(:editor)).parts.map { |part| part.body.to_s }.join
  ...
  test "the invitation is addressed with the site's own name rather than a hardcoded one" do
    ...
    assert_equal "You've been invited to Acme Docs", Nibble::PasswordsMailer.invite(users(:editor)).subject
  end
  ...
  test "a site that has not named itself still gets a sensible subject" do
    ...
    assert_equal "You've been invited to the site", Nibble::PasswordsMailer.invite(users(:editor)).subject
  end

  test "the reset email renders with a working link" do
    body = Nibble::PasswordsMailer.reset(users(:editor)).parts.map { |part| part.body.to_s }.join
