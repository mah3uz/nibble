...
class Nibble::Cp::BulkMoveTest < ActionDispatch::IntegrationTest
  ...
  test "selected entries move under the chosen parent, and back to the top" do
    ...
    post "/cp/actions", params: { resource: "collections.docs", handle: "move", ids: [ setup.id, faq.id ],
                                     values: { parent_id: guides.id.to_s } }
    assert_equal [ guides.id, guides.id ], [ setup.reload.parent_id, faq.reload.parent_id ]

    post "/cp/actions", params: { resource: "collections.docs", handle: "move", ids: [ setup.id ],
  ...
  test "a move with nowhere chosen changes nothing" do
    ...
    post "/cp/actions", params: { resource: "collections.docs", handle: "move", ids: [ setup.id ], values: {} }
