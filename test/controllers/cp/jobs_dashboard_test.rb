...
class Nibble::Cp::JobsDashboardTest < ActionDispatch::IntegrationTest
  ...
  test "retrying puts a failed job back in its queue" do
    ...
    post "/cp/utilities/jobs/#{failure.id}/retry"
  ...
  test "discarding removes a failed job for good" do
    ...
    delete "/cp/utilities/jobs/#{failure.id}"
