...
class Nibble::Cp::UpdatesControllerTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "loading more brings the next ten to append, without reading the feed again from the top" do
    ...
    get "/cp/updates", params: { page: 3 }, headers: {
      "X-Inertia" => "true", "X-Inertia-Version" => version.to_s,
      "X-Inertia-Partial-Component" => "cp/Updates", "X-Inertia-Partial-Data" => "releases,page,more"
    }
