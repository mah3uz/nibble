...
class Nibble::Cp::NibbleApi::RelationshipsControllerTest < ActionDispatch::IntegrationTest
  class FakeResolver
  ...
  test "pickers search through the registered resolver with the field's scope and a capped limit" do
    ...
    get cp_nibble_relationships_path("entry"), params: { q: " abo ", scope: { collections: [ "pages" ] }, limit: 500 }, as: :json
  ...
  test "an unknown record type is a 404, not a server error" do
    sign_in_as users(:editor)
    get cp_nibble_relationships_path("gadget"), as: :json
    assert_response :not_found
  end

  test "signed-out visitors can't search content" do
    get cp_nibble_relationships_path("entry"), as: :json
