...
class Nibble::Cp::TwoFactorTest < ActionDispatch::IntegrationTest
  ...
  test "a password alone doesn't sign in an account with two-factor on" do
    ...
    post "/cp/session", params: { email_address: users(:admin).email_address, password: "password" }

    assert_redirected_to "/cp/session/challenge"
    get "/cp"
    assert_redirected_to "/cp/session/new", "the password alone left them signed out"
  end
  ...
  test "a code from the authenticator finishes the sign-in, and can't be replayed" do
    ...
    post "/cp/session/challenge", params: { code: }
    assert_redirected_to "/cp"

    delete "/cp/session"
    post "/cp/session", params: { email_address: users(:admin).email_address, password: "password" }
    post "/cp/session/challenge", params: { code: }
    assert_redirected_to "/cp/session/challenge", "a used code is refused"
  end
  ...
  test "a recovery code works once and is then spent" do
    ...
    post "/cp/session/challenge", params: { code: codes.first }
    assert_redirected_to "/cp"
  ...
  test "five wrong codes end the attempt, so a stolen password can't be brute-forced" do
    ...
    post "/cp/session/challenge", params: { code: code_for(users(:admin)) }
    assert_redirected_to "/cp/session/new", "the pending sign-in is gone"
  end
  ...
  test "a role that requires two-factor holds the user on their account screen until it's on" do
    ...
    get "/cp/collections/posts"
    assert_redirected_to "/cp/account/edit"
  ...
  test "setting up two-factor shows the key once and turns on only after a correct code" do
    ...
    post "/cp/account/two_factor/confirm", params: { code: "000000" }
    follow_redirect!
  ...
  test "turning two-factor off needs the password, and a role that requires it refuses" do
    ...
    delete "/cp/account/two_factor", params: { current_password: "wrong" }
    assert users(:editor).reload.totp?
