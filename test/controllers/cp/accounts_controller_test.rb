...
class Nibble::Cp::AccountsControllerTest < ActionDispatch::IntegrationTest
  def errors = session[:inertia_errors].to_h.with_indifferent_access

  test "renaming yourself" do
    sign_in_as users(:editor)
    patch cp_account_path, params: { user: { name: "New Name" } }
  ...
  test "a blank name is rejected" do
    sign_in_as users(:editor)
    patch cp_account_path, params: { user: { name: "" } }
    assert_redirected_to edit_cp_account_path
  ...
  test "changing your password requires the current one" do
    sign_in_as users(:editor)
    patch cp_account_path, params: { user: { current_password: "wrong", password: "Test-Password-1", password_confirmation: "Test-Password-1" } }
    assert_redirected_to edit_cp_account_path
  ...
  test "changing your password signs out every other session but keeps this one" do
    ...
    this_session = Nibble::Current.session

    patch cp_account_path, params: { user: { current_password: "password", password: "Test-Password-1", password_confirmation: "Test-Password-1" } }
    assert_redirected_to edit_cp_account_path
  ...
  test "mismatched confirmation is refused" do
    sign_in_as users(:editor)
    patch cp_account_path, params: { user: { current_password: "password", password: "Test-Password-1", password_confirmation: "does-not-match" } }
    assert_redirected_to edit_cp_account_path
