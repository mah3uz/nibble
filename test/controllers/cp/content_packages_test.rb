...
class Nibble::Cp::ContentPackagesTest < ActionDispatch::IntegrationTest
  ...
  def import(dir, **params) = process(:post, "/cp/utilities/content/import", params: { package: zipped(dir), mode: "create", **params })

  test "export downloads the site's content as a zip another site can import" do
    Nibble::Packages::Importer.new(DEMO).call

    get "/cp/utilities/content/export"
  ...
  test "checking a package reports what it would do and writes nothing" do
    ...
    assert_redirected_to "/cp/utilities/content"
    assert report[:ok]
  ...
  test "importing waits for a fresh password, since it rewrites content across the site" do
    ...
    assert_redirected_to content_cp_utilities_path
    assert_equal 0, Nibble::Records::Entry.count

    follow_redirect!
    assert_equal "cp/Confirm", JSON.parse(Nokogiri::HTML(response.body).at_css("script[data-page]").text)["component"]
  end
  ...
  test "an upload that isn't a zip is turned away with a plain reason" do
    process :post, "/cp/utilities/content/import",
