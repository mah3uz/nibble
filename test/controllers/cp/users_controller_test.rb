...
class Nibble::Cp::UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "nobody edits their own roles, so an administrator can't quietly demote or promote themselves" do
    patch "/cp/users/#{users(:admin).id}", params: { user: { name: "Renamed", role_ids: [ roles(:author).id ] } }
  ...
  test "a user can hold several roles and gets the union of them" do
    patch "/cp/users/#{users(:author).id}", params: { user: { role_ids: [ roles(:author).id, roles(:editor).id ] } }
