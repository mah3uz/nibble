...
class Admin::MediaControllerTest < ActionDispatch::IntegrationTest
  ...
  test "the library lists the current folder's assets and subfolders, and searching looks everywhere" do
    ...
    body = json(:get, "/admin/media")
    assert_equal [ root.id ], body["listing"]["rows"].map { |row| row["id"] }
    assert_equal [ "team" ], body["folders"].map { |folder| folder["path"] }
    assert_equal [ "team/2026" ], json(:get, "/admin/media?folder=team")["folders"].map { |folder| folder["path"] }
    assert_equal 2, json(:get, "/admin/media?q=photo")["listing"]["rows"].size
  end

  test "the page size is always one the per-page menu offers, so the menu never shows blank" do
    pagination = json(:get, "/admin/media")["listing"]["pagination"]
    assert_includes pagination["per_page_options"], pagination["per_page"]

    pagination = json(:get, "/admin/media?per_page=7")["listing"]["pagination"]
  ...
  test "the unused filter is the unused-asset report, and it leaves out anything content still uses" do
    ...
    assert_equal [ unused.id ], json(:get, "/admin/media?usage=unused")["listing"]["rows"].map { |row| row["id"] }
    assert_equal [ used.id ], json(:get, "/admin/media?usage=used")["listing"]["rows"].map { |row| row["id"] }
  end

  test "an upload becomes an asset in the folder it was dropped into" do
    blob = upload_blob

    body = json(:post, "/admin/media", { signed_id: blob.signed_id, folder: "photos" })
  ...
  test "renaming keeps the extension, so a file can't be renamed into another type" do
    ...
    json(:patch, "/admin/media/#{asset.id}", { asset: { filename: "Holiday Snap.php" } })
  ...
  test "reupload swaps the file behind the same asset, so every reference and URL follows it" do
    ...
    json(:post, "/admin/media/#{asset.id}/reupload", { signed_id: blob.signed_id })
    asset.reload
  ...
  test "replace points every use at another asset, live and in drafts, and can trash the original" do
    ...
    body = json(:post, "/admin/media/#{old.id}/replace", { with: fresh.id, delete_original: true })
  ...
  test "bulk actions move, tag, duplicate and trash many assets at once" do
    ...
    json(:post, "/admin/media/bulk", { handle: "move", ids:, folder: "archive" })
    json(:post, "/admin/media/bulk", { handle: "tag", ids:, tag: "2026" })
    assert_equal [ [ "archive", [ "2026" ] ] ] * 2, Asset.where(id: ids).map { |asset| [ asset.folder, asset.tags ] }

    assert_difference -> { Asset.count }, 2 do
      json(:post, "/admin/media/bulk", { handle: "duplicate", ids: })
    end
    json(:post, "/admin/media/bulk", { handle: "trash", ids: })
  ...
  test "save as copy makes a new asset with the edits, sharing the stored file" do
    ...
    body = json(:post, "/admin/media/#{asset.id}/duplicate", { asset: { edits: { rotate: 180 } } })
  ...
  test "folders are created, renamed with everything inside, and only deleted when empty" do
    ...
    json(:delete, "/admin/media/folders?path=people")
  ...
  test "authors can upload but can't edit or delete assets" do
    ...
    json(:post, "/admin/media", { signed_id: upload_blob.signed_id })
    assert_response :created
    json(:patch, "/admin/media/#{asset.id}", { asset: { alt: "x" } })
    assert_response :forbidden
    json(:post, "/admin/media/bulk", { handle: "trash", ids: [ asset.id ] })
    assert_response :forbidden
  end
  ...
  test "the editor gets the asset's fields, usage and folders" do
    ...
    body = json(:get, "/admin/media/#{asset.id}")
  ...
  test "trashed assets show in the trash, restore from it, and purging deletes the file" do
    ...
    post "/admin/trash/asset-#{asset.id}/restore"
    assert_not asset.reload.trashed?
    lifecycle(asset, :trash)
    assert_enqueued_with(job: ActiveStorage::PurgeJob) { delete "/admin/trash/asset-#{asset.id}/purge" }
