...
class Admin::FormsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "the forms index counts real submissions per form and what's still unread; spam doesn't count" do
    ...
    assert_includes props["admin"]["nav"].flat_map { |section| section["items"] }.map { |item| item["title"] }, "Forms"
  end
  ...
  test "a form's listing hides spam unless asked, searches submitted values, and pages newest first" do
    ...
    get "/admin/forms/contact"
    assert_equal "admin/forms/Show", page["component"]
    ...
    assert_equal [ "/admin/forms/contact/submissions/#{Submission.find_by!(status: 'received', created_at: ...1.5.days.ago).id}" ],
      [ listing["rows"].last["edit_url"] ]

    get "/admin/forms/contact", params: { status: "spam" }
    assert_equal [ "Spammer" ], props["listing"]["rows"].map { |row| row["name"] }

    get "/admin/forms/contact", params: { q: "GRACE" }
  ...
  test "submissions export to CSV, all of them or only what the listing is filtered to" do
    ...
    get "/admin/forms/contact.csv", params: { q: "grace" }
  ...
  test "opening a submission marks it read and shows it as a read-only form, with its files and deliveries" do
    ...
    get "/admin/forms/contact/submissions/#{submission.id}"
    assert_equal "admin/forms/Submission", page["component"]
    ...
    get "/admin/forms/lead/submissions/#{submission.id}"
    ...
    get "/admin/forms/application/submissions/#{application.id}"
    assert_equal [ { "filename" => "cv.pdf", "size" => 10, "url" => "/admin/forms/application/submissions/#{application.id}/files/7" } ],
      props["meta"]["cv"]["files"]
  end
  ...
  test "a failed delivery can be retried from its submission, and only a failed one" do
    ...
    post "/admin/forms/contact/submissions/#{submission.id}/deliveries/api.0/retry"
    assert_equal "pending", submission.reload.delivery("api.0")["status"]
    assert_enqueued_with(job: Nibble::Jobs::DeliverSubmission)

    post "/admin/forms/contact/submissions/#{submission.id}/deliveries/api.1/retry"
  ...
  test "deleting submissions, one or in bulk, takes their files and delivery logs with them" do
    ...
    perform_enqueued_jobs { post "/admin/actions", params: { resource: "forms.contact", handle: "delete", ids: [ doomed.id ] } }
    ...
    delete "/admin/forms/contact/submissions/#{kept.id}"
    assert_redirected_to "/admin/forms/contact"
  ...
  test "a new-submission notification links to the submission" do
    ...
    assert_equal "/admin/forms/contact/submissions/#{submission.id}", response.parsed_body["notifications"].first["url"]
  end
end
