...
class Admin::WebhooksControllerTest < ActionDispatch::IntegrationTest
  ...
  def create_webhook(**attrs) = post("/admin/webhooks", params: { webhook: { name: "Site", url: HOOK, events: [ "record.published" ], enabled: true }.merge(attrs) })
  ...
  test "creating a webhook gives it its own signing secret; bad input comes back on the fields" do
    ...
    assert_redirected_to "/admin/webhooks/#{webhook.id}/edit"
  ...
  test "a test ping reports the receiver's answer straight away" do
    ...
    get "/admin/webhooks/#{webhook.id}/edit"
  ...
  test "a webhook that turned itself off can be turned back on, with its failure count cleared" do
    ...
    post "/admin/webhooks/#{webhook.id}/enable"
  ...
  test "a new secret replaces the old one, and a delivery can be resent" do
    ...
    webhook = Webhook.sole
    old = webhook.secret
    post "/admin/webhooks/#{webhook.id}/roll_secret"
    ...
    assert_enqueued_with(job: Nibble::Jobs::DeliverWebhook) { post "/admin/webhooks/#{webhook.id}/deliveries/#{delivery.id}/resend" }
