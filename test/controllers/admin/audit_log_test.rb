...
class Admin::AuditLogTest < ActionDispatch::IntegrationTest
  ...
  test "following the cursor walks the whole log with nothing repeated or skipped" do
    ...
    seen.concat(ids)
    while next_cursor
      get "/admin/utilities/audit", params: { before: next_cursor }
      seen.concat(ids)
    end
  ...
  test "an entry recorded between loads doesn't shift the next page" do
    ...
    record(1)
    get "/admin/utilities/audit", params: { before: cursor }
