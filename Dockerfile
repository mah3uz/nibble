...
# Production image. Deploy with Kamal (config/deploy.yml) or build and run by hand:
#   docker build -t nibble .
#   docker run -d -p 80:80 -e RAILS_MASTER_KEY=<config/master.key> -v nibble-storage:/rails/storage nibble
#
...
# Install application gems; Nibble's Gemfile and package.json are read from its folder
COPY vendor/nibble ./vendor/nibble
...
# -j 1 disable parallel compilation to avoid a QEMU bug: https://github.com/rails/bootsnap/issues/495
RUN bundle exec bootsnap precompile -j 1 app/ lib/ vendor/nibble/app/ vendor/nibble/lib/

# A theme named by the environment rather than config/nibble.yml has to be named here too, or the build would
# bundle a different theme from the one the server renders.
ARG NIBBLE_THEME
