...
const avatar = computed(() => {
  const seed = [...(admin.value.user?.email_address ?? '')].reduce(
...
const signOut = () => router.delete('/admin/session')
</script>
  ...
  <DropdownMenu>
    ...
    <DropdownMenuContent align="end" class="w-auto min-w-64 overflow-hidden bg-gray-50 p-0 dark:bg-gray-800">
      ...
      >
        ...
        >
        <span class="min-w-0">
          <span class="block truncate">{{ admin.user?.email_address }}</span>
          <span v-if="admin.user?.role" class="block truncate text-xs font-normal text-gray-500">{{
            admin.user.role
          }}</span>
        </span>
      </header>
      ...
      <footer class="px-1.75 py-2">
        <ToggleGroup
          ...
          @update:model-value="(value) => value && (theme = value as AdminPreferences['theme'])"
        >
          <ToggleGroupItem
