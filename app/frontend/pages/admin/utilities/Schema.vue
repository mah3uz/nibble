...
useBreadcrumbs([{ label: 'Utilities', url: '/admin/utilities' }, { label: 'Schema' }])
</script>
...
<template>
  <div>
    <PageHeader
      title="Schema"
      icon="blueprints"
      :breadcrumbs="[{ label: 'Utilities', url: '/admin/utilities' }, { label: 'Schema' }]"
    >
      <template #actions>
        <Button variant="outline" as-child><Link href="/admin/blueprints">Browse blueprints</Link></Button>
      </template>
    </PageHeader>

    <AdminPanel title="nibble:check" :flush="checks.length > 0">
      <template #actions>
    ...
    <div class="mb-6 grid gap-6 md:grid-cols-2">
      <AdminPanel v-for="group in groups" :key="group" :title="LABELS[group]" fill>
        ...
        <ul v-else class="flex flex-wrap gap-2">
          ...
          >
            <AdminIcon :name="group === 'navigations' ? 'navigation' : group" class="size-3.5 text-gray-500" />
