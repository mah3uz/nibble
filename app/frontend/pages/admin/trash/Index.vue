...
async function purge(item: TrashItem) {
  ...
  if (ok) router.delete(`/admin/trash/${item.id}/purge`)
}
</script>
  ...
  <div class="mx-auto max-w-5xl">
    <div v-if="!items.length" class="mx-auto max-w-md py-14">
      <h1 class="mb-8 flex items-center justify-center gap-3 text-[25px] font-medium">
        <AdminIcon name="trash" class="size-5 text-gray-500" />Trash
      </h1>
          ...
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="actions-column">
              <div class="flex justify-end gap-2">
                <Button variant="outline" size="sm" @click="router.post(`/admin/trash/${item.id}/restore`)">
                  Restore
                </Button>
