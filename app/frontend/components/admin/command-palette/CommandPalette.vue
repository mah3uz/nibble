...
import { NAV_ICONS } from '@/components/admin/shell/icons'
...
const navCommands = computed<PaletteCommand[]>(() =>
  admin.value.nav.flatMap((section) =>
...
const THEME_CYCLE: AdminPreferences['theme'][] = ['system', 'light', 'dark']
...
const actionCommands = computed<PaletteCommand[]>(() => {
  ...
  for (const collection of admin.value.collections) {
              ...
              <ListboxGroup v-for="group in groups" :key="group.label" class="space-y-1 px-3 py-2">
                ...
                >
                  <div class="flex size-6 shrink-0 items-center justify-center p-1 text-gray-500">
                    <AdminIcon v-if="typeof command.icon === 'string'" :name="command.icon" class="size-4" />
