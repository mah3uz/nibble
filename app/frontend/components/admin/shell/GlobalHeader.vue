...
<template>
  <header class="fixed inset-x-0 top-0 z-30 flex h-14 items-center gap-2 bg-header px-2 text-header-foreground lg:px-4">
    <div class="flex min-w-0 flex-1 items-center gap-3.5 text-[0.8125rem] text-white/85">
      <div class="flex shrink-0 items-center gap-1.5 sm:gap-2.5">
        <button
          ...
          @click="toggleSidebar"
        >
          <AdminIcon name="hamburger" class="size-3.5 sm:size-3.25" />
        </button>
        <Link
          href="/admin"
          class="flex items-center gap-1.5 rounded-xs whitespace-nowrap text-white/85 hover:text-white"
        >
    ...
    <div class="flex shrink-0 items-center justify-end gap-1 sm:gap-3 md:gap-2">
      ...
      >
        <AdminIcon name="magnifying-glass" class="size-4 text-white/50 group-hover:text-white/70" />
      ...
      >
        <AdminIcon name="layout-expand" :class="['size-4', expanded && 'text-white']" />
      </button>
      <a
        :href="admin.site_url"
        target="_blank"
      ...
      >
        <AdminIcon name="view-site" class="size-4" />
      </a>
      <NotificationBell />
      <UserMenu />
    </div>
