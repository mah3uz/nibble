...
import { UPDATE_DEBOUNCE_MS } from '@/nibble-admin/fieldtypes/useFieldtype'
...
async function load(id: number) {
  const data = await request<AssetDetail>('GET', `/admin/media/${id}`)
  detail.value = data
...
async function save() {
  ...
  try {
    await request('PATCH', `/admin/media/${asset.value.id}`, {
      asset: {
        ...values.value,
...
async function saveCopy(next: ImageEdits) {
  if (!asset.value) return
  try {
    await request('POST', `/admin/media/${asset.value.id}/duplicate`, { asset: { edits: next } })
      ...
      >
        ...
        <div class="asset-editor relative flex h-full flex-col rounded-sm bg-gray-100 dark:bg-gray-850">
          <header class="relative flex w-full justify-between px-2">
            ...
            >
              <AdminIcon name="folder-photos" class="size-5" />
              <span
          ...
          >
            <div v-if="asset" class="hidden h-full flex-1 gap-2 py-1 sm:flex sm:gap-3">
              <Badge v-if="asset.width"
                ><AdminIcon name="assets" class="size-3.5 opacity-60" /> {{ asset.width }} × {{ asset.height }}</Badge
              >
              <Badge><AdminIcon name="memory" class="size-3.5 opacity-60" /> {{ formatBytes(asset.size) }}</Badge>
              <Badge
                ><AdminIcon name="fingerprint" class="size-3.5 opacity-60" />
            ...
            <div class="flex items-center space-x-3">
              ...
              /></Button>
              <Button v-if="detail?.can.edit" :disabled="saving" @click="save"><AdminIcon name="save" /> Save</Button>
            </div>
          </div>
