...
import ListingFilters from '@/components/admin/listing/ListingFilters.vue'
import ListingPagination from '@/components/admin/listing/ListingPagination.vue'
import ListingSearch from '@/components/admin/listing/ListingSearch.vue'
import ListingTable from '@/components/admin/listing/ListingTable.vue'
...
async function onFolderDrop(path: string, event: DragEvent) {
  ...
  try {
    await Promise.all(ids.map((id) => request('PATCH', `/admin/media/${id}`, { asset: { folder: path } })))
...
async function createFolder() {
  ...
  if (!result) return
  try {
    await request('POST', '/admin/media/folders', { parent: props.data.folder, name: result.name })
    emit('changed')
  } catch (error) {
    toast.error((error as Error).message)
  }
...
async function renameFolder(path: string) {
  ...
  if (!result) return
  try {
    await request('PATCH', '/admin/media/folders', { path, name: result.name })
    emit('changed')
  } catch (error) {
    toast.error((error as Error).message)
  }
...
async function deleteFolder(path: string) {
  ...
  if (!ok) return
  try {
    await request('DELETE', `/admin/media/folders?path=${encodeURIComponent(path)}`)
    emit('changed')
  } catch (error) {
    toast.error((error as Error).message)
  }
...
async function bulk(handle: string, extra: Record<string, unknown> = {}) {
  try {
    const result = await request<{ updated: number }>('POST', '/admin/media/bulk', {
      handle,
...
<template>
  <div class="relative" @dragenter="onDragEnter" @dragleave="onDragLeave" @dragover.prevent @drop.prevent="onDrop">
    <div v-if="dragging" class="drag-notification">
      <AdminIcon name="upload-cloud-large" class="size-8" />
    ...
    <PageHeader v-if="mode === 'page'" title="Assets" icon="assets">
      <template #actions>
        <Button v-if="data.can.upload" variant="outline" @click="fileInput?.click()"
          ><AdminIcon name="upload" /> Upload</Button
        >
        <Button v-if="data.can.upload" variant="outline" @click="createFolder"
          ><AdminIcon name="add-folder" /> Create Folder</Button
        >
        <ToggleGroup
          type="single"
          :model-value="view"
        ...
        >
          <ToggleGroupItem value="grid" aria-label="Toggle grid"><AdminIcon name="layout-grid" /></ToggleGroupItem>
          <ToggleGroupItem value="table" aria-label="Toggle table"><AdminIcon name="layout-list" /></ToggleGroupItem>
        </ToggleGroup>
      </template>
    </PageHeader>
    ...
    >
      <header class="flex items-center justify-between gap-2 px-1 py-3">
        <nav class="flex flex-wrap items-center" aria-label="Folders">
          <Button variant="ghost" class="h-8! gap-2" @click="openFolder('')"><AdminIcon name="home" /> All</Button>
        ...
        <div v-if="view === 'grid'" class="mr-2 flex items-center gap-2">
          <Button
            ...
            ><AdminIcon name="eye-slash" class="size-4"
          /></Button>
          <SliderRoot
