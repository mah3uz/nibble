...
export function useAssetActions(options: {
  ...
  async function destroy(asset: Target) {
    ...
    if (!ok) return
    try {
      await request('DELETE', `/admin/media/${asset.id}`)
    } catch (error) {
      ...
      if (!force) return
      await request('DELETE', `/admin/media/${asset.id}?force=true`)
    }
  ...
  async function run(action: AssetMenuAction, asset: Target) {
    switch (action) {
      ...
      case 'duplicate':
        return attempt(() => request('POST', `/admin/media/${asset.id}/duplicate`), 'Duplicated')
      case 'move': {
        ...
        return attempt(() => request('PATCH', `/admin/media/${asset.id}`, { asset: { folder } }), 'Moved')
      }
      case 'rename': {
        const result = await confirm({
          title: 'Rename',
        ...
        if (!result) return
        return attempt(
          () => request('PATCH', `/admin/media/${asset.id}`, { asset: { filename: result.filename } }),
          'Renamed',
        )
      }
      case 'replace': {
        ...
        return attempt(async () => {
          await request('POST', `/admin/media/${asset.id}/replace`, {
            with: replacement.id,
      ...
      case 'reupload': {
        ...
        return attempt(async () => {
          ...
          await request('POST', `/admin/media/${asset.id}/reupload`, { signed_id: signedId })
        }, 'File replaced')
      }
      case 'delete':
