module Admin
  class RolesController < BaseController
    ...
    def index
      render inertia: "admin/roles/Index", props: { roles: Role.order(:title).map { |role| row(role) } }
    end

    def new
      render inertia: "admin/roles/Edit", props: form_props(Role.new)
    end

    def create
      return refuse_ungrantable("/admin/roles/new") if ungrantable.any?

      role = Role.new(role_attributes)
      return redirect_to("/admin/roles", notice: "Role created.") if role.save

      redirect_to "/admin/roles/new", inertia: { errors: messages(role) }
    end

    def edit
      render inertia: "admin/roles/Edit", props: form_props(@role)
    end

    def update
      return refuse_ungrantable("/admin/roles/#{@role.id}/edit") if ungrantable.any?
      return redirect_to("/admin/roles", notice: "Role updated.") if @role.update(role_attributes(@role))

      redirect_to "/admin/roles/#{@role.id}/edit", inertia: { errors: messages(@role) }
    end

    def destroy
      return redirect_to("/admin/roles", notice: "Role deleted.") if @role.destroy

      redirect_to "/admin/roles", alert: @role.errors.full_messages.to_sentence
    end
    ...
    def form_props(role)
      { role: { id: role.id, title: role.title, handle: role.handle, superuser: role.superuser?,
        ...
        can_assign_superuser: Current.user.admin? }
    end

    def row(role)
      { id: role.id, title: role.title, handle: role.handle, superuser: role.superuser?,
        abilities: role.abilities.size, users: role.users.count, edit_url: "/admin/roles/#{role.id}/edit" }
    end

    def role_attributes(role = nil)
      attrs = params.require(:role).permit(:title, :handle, :superuser, :require_2fa, abilities: [])
      superuser = (ActiveModel::Type::Boolean.new.cast(attrs[:superuser]) || false) && Current.user.admin?
