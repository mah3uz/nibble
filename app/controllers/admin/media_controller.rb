module Admin
  class MediaController < BaseController
    ...
    def index
      listing = Nibble::Cp::AssetListing.new(user: Current.user, params:)
    ...
    def show
      ...
      render json: {
        asset: Nibble::Cp::AssetListing.new(user: Current.user).row(@asset, usage.size).merge(
        ...
        usage:,
        folder_options: Nibble::Cp::AssetListing.new(user: Current.user).folder_options,
        can: abilities
      }
    end
    ...
    def create
      ...
      respond(Nibble::Assets::Upload.call(blob, { "folder" => params[:folder].to_s }, actor: Current.user), status: :created)
    end

    def update
      authorize!("assets.edit")
      respond(Nibble::Lifecycle.call(@asset, :save, attrs, actor: Current.user))
    end

    def destroy
      authorize!("assets.delete")
      result = Nibble::Lifecycle.call(@asset, :trash, { "force" => params[:force].to_s == "true" }, actor: Current.user)
    ...
    def duplicate
      authorize!("assets.upload")
      respond(Nibble::Lifecycle.call(duplicate_of(@asset), :create, copied_values(@asset).merge(attrs), actor: Current.user), status: :created)
    end

    def reupload
      authorize!("assets.edit")
      blob = ActiveStorage::Blob.find_signed(params.require(:signed_id)) or raise ActiveRecord::RecordNotFound
      respond(Nibble::Assets::Upload.replace(@asset, blob, actor: Current.user))
    end
    ...
    def replace
      ...
      failures = sources.filter_map do |source|
        result = Nibble::Lifecycle.call(source, :replace_asset, { "from" => @asset.id, "to" => replacement.id }, actor: Current.user)
        "#{usage_title(source)}: #{result.errors.values.flatten.first}" unless result.ok?
      end
      Nibble::Lifecycle.call(@asset, :trash, { "force" => true }, actor: Current.user) if failures.empty? && params[:delete_original].to_s == "true"
    ...
    def bulk_change(asset)
      case params[:handle]
      when "move" then Nibble::Lifecycle.call(asset, :save, { "folder" => params[:folder].to_s }, actor: Current.user)
      when "tag" then Nibble::Lifecycle.call(asset, :save, { "tags" => asset.tags + [ params.require(:tag).to_s ] }, actor: Current.user)
      when "duplicate" then Nibble::Lifecycle.call(duplicate_of(asset), :create, copied_values(asset), actor: Current.user)
      else Nibble::Lifecycle.call(asset, :trash, { "force" => true }, actor: Current.user)
      end
    end
    ...
    def respond(result, status: :ok)
      ...
      render json: { asset: Nibble::Cp::AssetListing.new(user: Current.user).row(result.record, 0) }, status:
    end

    def abilities
      %w[upload edit delete].index_with { |action| Nibble::Access.can?(Current.user, "assets.#{action}") }
    end
    ...
    def edit_url(source)
      case source
      when Nibble::Records::Entry then "/admin/collections/#{source.collection}/entries/#{source.id}/edit"
      when Nibble::Records::Term then "/admin/taxonomies/#{source.taxonomy}/terms/#{source.id}/edit"
      when Nibble::Records::GlobalSet then "/admin/globals/#{source.handle}/edit"
      when Nibble::Records::NavigationTree then "/admin/navigation/#{source.handle}/edit"
      end
    end
  end
end
