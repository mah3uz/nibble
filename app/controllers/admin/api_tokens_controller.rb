module Admin
  class ApiTokensController < BaseController
    ...
    def index
      render inertia: "admin/api_tokens/Index", props: {
    ...
    def create
      ...
      token, plaintext = ApiToken.issue(name: attrs[:name], scopes: Array(attrs[:scopes]),
        expires_at: EXPIRIES[attrs[:expires_in].to_s]&.from_now, created_by: Current.user)
      flash[:issued_token] = { name: token.name, token: plaintext }
      redirect_to "/admin/api-tokens", notice: "Token created. It's shown only once."
    rescue ActiveRecord::RecordInvalid => e
      redirect_to "/admin/api-tokens", inertia: { errors: e.record.errors.to_hash(true).transform_values { |list| list.join(", ") } }
    end

    def destroy
      ApiToken.find(params[:id]).update!(revoked_at: Time.current)
      redirect_to "/admin/api-tokens", notice: "Token revoked."
    end
