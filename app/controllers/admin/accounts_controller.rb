module Admin
  class AccountsController < BaseController
    ...
    def edit
      render inertia: "admin/account/Edit", props: { two_factor: two_factor_props, passkeys: passkey_props }
    end
    ...
    def update_name(attrs)
      if Current.user.update(attrs.permit(:name))
        redirect_to edit_admin_account_path, notice: "Account updated."
      else
        redirect_to edit_admin_account_path, inertia: { errors: messages(Current.user) }
      end
    end

    def update_password(attrs)
      unless Current.user.authenticate(attrs[:current_password])
        return redirect_to edit_admin_account_path, inertia: { errors: { current_password: "is incorrect" } }
      end

      if Current.user.update(attrs.permit(:password, :password_confirmation))
        Current.user.sessions.where.not(id: Current.session.id).destroy_all
        redirect_to edit_admin_account_path, notice: "Password changed. Your other sessions were signed out."
      else
        redirect_to edit_admin_account_path, inertia: { errors: messages(Current.user) }
      end
    end
    ...
    def passkey_props
      Current.user.user_credentials.order(:created_at).map do |credential|
