module Admin
  class FormSubmissionsController < BaseController
    ...
    def show
      ...
      render inertia: "admin/forms/Submission", props: {
        ...
        meta:,
        can_delete: Nibble::Access.can?(Current.user, "forms.#{@form.handle}.delete")
      }
    end
    ...
    def destroy
      ...
      @submission.destroy!
      redirect_to "/admin/forms/#{@form.handle}", notice: "Submission deleted."
    end
    ...
    def retry
      ...
      redirect_back_or_to "/admin/forms/#{@form.handle}/submissions/#{@submission.id}", notice: "Delivery queued to retry."
    end
    ...
    def file_row(file)
      { "filename" => file["filename"], "size" => file["size"],
        "url" => "/admin/forms/#{@form.handle}/submissions/#{@submission.id}/files/#{file['id']}" }
    end

    def delivery_row(delivery)
      delivery.slice("key", "target", "mode", "status", "attempts", "response_status", "error", "errors", "at")
        .merge("can_retry" => delivery["status"] == "failed" && Nibble::Access.can?(Current.user, "forms.#{@form.handle}.edit"))
    end
  end
end
