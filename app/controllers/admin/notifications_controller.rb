module Admin
  class NotificationsController < BaseController
    ...
    def scope = Nibble::Records::Notification.where(user_id: Current.user.id)
    ...
    def url_for_subject(item)
      return "/admin/forms/#{item.data['form']}/submissions/#{item.subject_id}" if item.kind == "form.submitted" && item.subject_id
      return "/admin/webhooks/#{item.subject_id}/edit" if item.kind == "webhook.disabled"
      ...
      "/admin/collections/#{entry.collection}/entries/#{entry.id}/edit"
    end
  end
end
