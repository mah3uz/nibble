# Nibble sign-in (Rails 8 authentication), rendered with the admin Inertia UI.
class SessionsController < ApplicationController
  ...
  def new
    render inertia: "admin/auth/Login", props: { flash: flash.to_hash.slice("notice", "alert") }
  end
  ...
  def elevate
    Nibble::Lockout.clear(Current.user.email_address)
    elevate_session!
    Nibble::AuthLog.record("elevated", user: Current.user, ip: request.remote_ip)
    head :ok
  end
  ...
  def challenge
    ...
    render inertia: "admin/auth/Challenge", props: { flash: flash.to_hash.slice("notice", "alert") }
  end

  def verify_challenge
    user = pending_user or return redirect_to(new_session_path, alert: "Sign in again.")
  ...
  def destroy
    Nibble::AuthLog.record("signed_out", user: Current.user, ip: request.remote_ip)
    terminate_session
  ...
  def require_elevated_password
    ...
    return if Current.user&.authenticate(params[:password].to_s)

    Nibble::Lockout.record_failure(email, ip: request.remote_ip)
    Nibble::AuthLog.record("elevation_refused", user: Current.user, ip: request.remote_ip)
    render json: { error: "That password isn't right." }, status: :unauthorized
  end

  def refuse_locked
    redirect_to new_session_path, alert: "Too many attempts. Try again in #{Nibble::Lockout::WINDOW.inspect}."
  end
  ...
  def start_challenge(user)
    ...
    redirect_to challenge_session_path
  end
  ...
  def refuse_code(user)
    ...
    if session[:pending_attempts] >= CODE_ATTEMPTS
      clear_challenge
      return redirect_to new_session_path, alert: "Too many codes. Sign in again."
    end

    redirect_to challenge_session_path, alert: "That code isn't right."
  end
end
