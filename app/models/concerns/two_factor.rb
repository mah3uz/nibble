module TwoFactor
  ...
  def totp_provisioning_uri
    ...
    ROTP::TOTP.new(secret, issuer: TwoFactor.issuer).provisioning_uri(email_address)
  end
  ...
  def generate_recovery_codes!
    ...
    update!(recovery_codes: codes.map { |code| TwoFactor.digest(code) })
    codes
  end

  def consume_recovery_code(code)
    digest = TwoFactor.digest(code.to_s.strip.downcase.delete("^a-z0-9"))
