module Admin
  class UtilitiesController < BaseController
    ...
    def show
      render inertia: "admin/utilities/Index", props: { utilities: Nibble::Cp::Utilities::LIST.map { |utility| utility.merge(url: Nibble::Cp::Utilities.url(utility)) } }
    end

    def cache
      render inertia: "admin/utilities/Cache", props: { store: Nibble::PageCache.store.class.name.demodulize.underscore.humanize }
    end

    def search
      render inertia: "admin/utilities/Search", props: { search: search_stats }
    end

    def schema
      render inertia: "admin/utilities/Schema", props: {
    ...
    def backups
      render inertia: "admin/utilities/Backups", props: { backups: backup_info }
    end
    ...
    def audit
      ...
      render inertia: "admin/utilities/Audit", props: { audit: InertiaRails.scroll(page) { audit_rows(rows.first(AUDIT_PAGE)) } }
    end

    REPORT_ERRORS = 15

    def content
      render inertia: "admin/utilities/Content", props: {
        can_export: Nibble::Access.can?(Current.user, "content.export"),
        can_import: Nibble::Access.can?(Current.user, "content.import"),
    ...
    def import_content
      ...
      return redirect_to(content_admin_utilities_path) if confirm && !elevated?
      ...
      redirect_to content_admin_utilities_path
    rescue Nibble::Error => e
      redirect_to content_admin_utilities_path, alert: e.message
    end
    ...
    def retry_job
      ...
      redirect_to jobs_admin_utilities_path, notice: "Job queued to run again."
    end

    def discard_job
      failed_execution.discard
      redirect_to jobs_admin_utilities_path, notice: "Job discarded."
    end

    def clear_cache
      Nibble::PageCache.store.clear
      redirect_to cache_admin_utilities_path, notice: "Page cache cleared."
    end

    def purge_cache
      tags = params[:tags].to_s.split(/[\s,]+/).compact_blank
      return redirect_to cache_admin_utilities_path, alert: "Name at least one cache tag." if tags.empty?

      Nibble::PageCache.purge(tags)
      redirect_to cache_admin_utilities_path, notice: "Purged #{tags.to_sentence}."
    end

    def rebuild_search
      authorize!("search.rebuild")
      Nibble::Search.rebuild
      redirect_to search_admin_utilities_path, notice: "Search indexes rebuilt."
    end
    ...
    def search_stats
      ...
      { indexes:, indexed:, total: Nibble::Records::Entry.live.where(collection: collections).count,
        can_rebuild: Nibble::Access.can?(Current.user, "search.rebuild") }
    end
