  ...
  class CommentsController < BaseController
    ...
    def create
      ...
      comment = Nibble::Records::Comment.create!(
        subject_type: @entry.class.name, subject_id: @entry.id, author_id: Current.user.id,
    ...
    def destroy
      ...
      return head :forbidden unless comment.author_id == Current.user.id
    ...
    def serialize(comment, authors)
      { "id" => comment.id, "body" => comment.body, "at" => comment.created_at.utc.iso8601,
        "author" => authors[comment.author_id]&.name, "mine" => comment.author_id == Current.user.id }
    end
    ...
    def notify(mentioned, comment)
      mentioned.each do |user|
        ...
        Nibble::Records::Notification.notify(user.id, "comment.mentioned", subject: @entry, title: @entry.title,
                                             comment_id: comment.id, by: Current.user.id)
      end
    end
  end
end
