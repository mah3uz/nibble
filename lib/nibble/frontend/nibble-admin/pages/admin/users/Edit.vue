...
async function save() {
  form.transform((data) => ({ user: data })).patch(`/admin/users/${props.user.id}`)
}

async function sendReset() {
  router.post(`/admin/users/${props.user.id}/send_reset`)
}
...
async function destroy() {
  ...
  if (ok) router.delete(`/admin/users/${props.user.id}`)
}
...
async function revoke(session: SessionRow) {
  ...
  if (ok) router.delete(`/admin/users/${props.user.id}/sessions/${session.id}`)
}
</script>
...
<template>
  <div class="mx-auto max-w-5xl">
    ...
    >
      <AdminIcon name="chevron-left" class="size-4" />Users
    </Link>
    <PageHeader
      :title="user.name"
      icon="users"
      :breadcrumbs="[{ label: 'Users', url: '/admin/users' }, { label: user.name }]"
    >
      <template #actions>
