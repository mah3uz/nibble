...
import PermissionNode from '@/components/admin/roles/PermissionNode.vue'
...
const base = computed(() => (props.role.id ? `/admin/roles/${props.role.id}` : null))
...
<template>
  <div class="mx-auto max-w-5xl">
    ...
    >
      <AdminIcon name="chevron-left" class="size-4" />Roles
    </Link>
    <PageHeader :title="title" icon="users" :breadcrumbs="[{ label: 'Roles', url: '/admin/roles' }, { label: title }]">
      <template #actions>
        ...
        >
          ...
          <div v-if="can_assign_superuser" class="grid gap-3 px-4 py-4 md:grid-cols-2 md:items-center">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Everything in the control panel, including anything added later. Grant it sparingly.
              </p>
            </div>
            <div class="space-y-1.5">
