  ...
  class SearchController < BaseController
    ...
    def collection_groups(term)
      Nibble.schema.collections.filter_map do |collection|
        next unless Nibble::Access.can?(Current.user, "entries.#{collection.handle}.view")
        ...
        group(collection["title"], records.map do |entry|
          item(entry.title, entry.uri, "/admin/collections/#{collection.handle}/entries/#{entry.id}/edit", "collection", entry.status)
        end)
      end
    end
    ...
    def taxonomy_groups(term)
      Nibble.schema.taxonomies.filter_map do |taxonomy|
        next unless Nibble::Access.can?(Current.user, "terms.#{taxonomy.handle}.view")

        records = matching(Nibble::Records::Term.kept.where(taxonomy: taxonomy.handle), term)
        group(taxonomy["title"], records.map do |record|
          item(record.title, record.uri, "/admin/taxonomies/#{taxonomy.handle}/terms/#{record.id}/edit", "taxonomy")
        end)
      end
    end
