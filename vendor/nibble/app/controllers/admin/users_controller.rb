  ...
  class UsersController < BaseController
    ...
    def index
      render inertia: "admin/users/Index", props: {
        listing: Nibble::Cp::UserListing.new(user: Current.user, params:).props,
        roles: role_options
      }
    end
    ...
    def create
      ...
      unless user.save
        return redirect_to "/admin/users", inertia: { errors: messages(user) }
      end

      PasswordsMailer.invite(user).deliver_later
      redirect_to "/admin/users", notice: "Invitation sent to #{user.email_address}."
    end

    def edit
      render inertia: "admin/users/Edit", props: {
        user: user_props(@user), roles: role_options, self: @user.id == Current.user.id,
    ...
    def update
      ...
      if @user.update(user_attributes)
        ...
        return redirect_to("/admin/users", notice: "User updated.")
      end

      redirect_to "/admin/users/#{@user.id}/edit", inertia: { errors: messages(@user) }
    end

    def destroy
      raise NotAuthorized if @user.id == Current.user.id
      return redirect_to("/admin/users", notice: "User deleted.") if @user.destroy

      redirect_to "/admin/users/#{@user.id}/edit", alert: @user.errors.full_messages.to_sentence
    end

    def send_reset
      PasswordsMailer.reset(@user).deliver_later
      redirect_to "/admin/users/#{@user.id}/edit", notice: "Password reset sent to #{@user.email_address}."
    end

    def revoke_session
      @user.sessions.find(params[:session_id]).destroy!
      redirect_to "/admin/users/#{@user.id}/edit", notice: "Session revoked."
    end
    ...
    def record_role_change(before)
      ...
      Nibble::AuthLog.record("roles_changed", user: @user, ip: request.remote_ip, by: Current.user.id, from: before, to: after)
    end
    ...
    def user_attributes
      ...
      @user.id == Current.user.id ? attrs : attrs.merge(roles: submitted_roles)
    end

    def submitted_roles
      roles = Role.where(id: Array(params[:user][:role_ids])).to_a
      raise NotAuthorized if roles.any?(&:superuser?) && !Current.user.admin?
    ...
    def only_a_superuser_touches_one
      raise NotAuthorized if @user.admin? && !Current.user.admin?
    end
    ...
    def session_props(session)
      { id: session.id, user_agent: session.user_agent, ip_address: session.ip_address,
        created_at: session.created_at.utc.iso8601, current: session.id == Current.session.id }
    end
