  ...
  module Cp
    class WebhooksController < BaseController
      ...
      def index
        ...
        render inertia: "cp/webhooks/Index", props: {
          webhooks: Nibble::Records::Webhook.order(:name).map do |webhook|
            ...
            { id: webhook.id, name: webhook.name, url: webhook.url, events: webhook.events, enabled: webhook.enabled,
              disabled_reason: webhook.disabled_reason, edit_url: "/cp/webhooks/#{webhook.id}/edit",
      ...
      def create
        ...
        return redirect_to("/cp/webhooks/new", inertia: { errors: errors_for(webhook) }) unless webhook.save

        redirect_to "/cp/webhooks/#{webhook.id}/edit", notice: "Webhook created. Copy its signing secret to verify deliveries."
      end

      def update
        return redirect_to("/cp/webhooks/#{@webhook.id}/edit", inertia: { errors: errors_for(@webhook) }) unless @webhook.update(webhook_params)

        redirect_to "/cp/webhooks/#{@webhook.id}/edit", notice: "Webhook saved."
      end

      def destroy
        @webhook.destroy!
        redirect_to "/cp/webhooks", notice: "Webhook deleted."
      end
      ...
      def test
        ...
        redirect_to "/cp/webhooks/#{@webhook.id}/edit", **flash
      end

      def enable
        @webhook.update!(enabled: true, consecutive_failures: 0, disabled_at: nil, disabled_reason: nil)
        redirect_to "/cp/webhooks/#{@webhook.id}/edit", notice: "Webhook turned back on."
      end

      def roll_secret
        @webhook.update!(secret: Nibble::Records::Webhook.generate_secret)
        redirect_to "/cp/webhooks/#{@webhook.id}/edit", notice: "New signing secret created. Update your receiver to use it."
      end

      def resend
        delivery = @webhook.deliveries.find(params[:delivery_id])
        Nibble::Webhooks.resend(delivery)
        redirect_to "/cp/webhooks/#{@webhook.id}/edit", notice: "Delivery queued to resend."
      end
      ...
      def render_form(webhook)
        render inertia: "cp/webhooks/Edit", props: {
