  ...
  module Cp
    class TrashController < BaseController
      def index
        ...
        render inertia: "cp/trash/Index", props: { items: entries + terms + assets }
      end

      def restore
        record = find_record
        authorize!(ability(record, "delete"), record)
        result = Nibble::Lifecycle.call(record, :restore, {}, actor: Nibble::Current.user)
        return redirect_to(cp_trash_index_path, alert: result.errors.values.flatten.first) unless result.ok?

        redirect_to cp_trash_index_path, notice: "Restored."
      end
      ...
      def purge
        ...
        record.destroy!
        redirect_to cp_trash_index_path, notice: "Deleted for good."
      end
      ...
      def entries
        Nibble::Records::Entry.where.not(deleted_at: nil).order(deleted_at: :desc).filter_map do |entry|
          next unless Nibble::Access.can?(Nibble::Current.user, "entries.#{entry.collection}.delete", entry)
      ...
      def terms
        Nibble::Records::Term.where.not(deleted_at: nil).order(deleted_at: :desc).filter_map do |term|
          next unless Nibble::Access.can?(Nibble::Current.user, "terms.#{term.taxonomy}.delete", term)
      ...
      def assets
        return [] unless Nibble::Access.can?(Nibble::Current.user, "assets.delete")
