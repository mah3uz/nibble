  ...
  module Cp
    class PasskeysController < BaseController
      ...
      def create
        ...
        Nibble::Current.user.user_credentials.create!(external_id: credential.id, public_key: credential.public_key,
      ...
      def update
        ...
        redirect_to edit_cp_account_path, notice: "Passkey renamed."
      end

      def destroy
        if last_factor?
          return redirect_to edit_cp_account_path,
            alert: "Your role asks for two-factor authentication, so set up another way in before removing this one."
        end

        passkey.destroy!
        Nibble::AuthLog.record("passkey_removed", user: Nibble::Current.user, ip: request.remote_ip)
        redirect_to edit_cp_account_path, notice: "Passkey removed."
      end

      private

      def passkey = Nibble::Current.user.user_credentials.find(params[:id])

      def last_factor?
        Nibble::Current.user.requires_two_factor? && !Nibble::Current.user.totp? && Nibble::Current.user.user_credentials.count == 1
      end

      # Every second factor needs a way back in, not just the authenticator app.
      def offer_recovery_codes
        return if Nibble::Current.user.recovery_codes.any?
