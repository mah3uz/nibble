  ...
  module Cp
    class CollectionsController < BaseController
      ...
      def show
        ...
        props = {
          collection: { handle: collection.handle, title: collection["title"], icon: collection["icon"] || "collections", structured:, dated:, view:,
                        ...
                        blueprints: Nibble.schema.blueprints_for(collection).map do |item|
                          { title: item["title"], url: "/cp/blueprints/#{collection.handle}.#{item.handle}" }
                        end },
          create: create_link(collection)
        }
        listing = Nibble::Cp::Listing.new(collection, user: Nibble::Current.user, params:)
        ...
        render inertia: "cp/collections/Index", props:
      end
      ...
      def create_link(collection)
        return nil if collection["files"].present? || !Nibble::Access.can?(Nibble::Current.user, "entries.#{collection.handle}.create")

        { label: "New #{collection['title'].to_s.singularize.downcase}", url: "/cp/collections/#{collection.handle}/entries/new" }
      end
        ...
        by_parent.fetch(parent_key, []).sort_by { |page| [ page.position || Float::INFINITY, page.title ] }.map do |page|
          {
            ...
            edit_url: "/cp/collections/#{collection.handle}/entries/#{page.id}/edit",
        ...
        by_parent.fetch(parent_id, []).map do |entry|
          {
            ...
            edit_url: "/cp/collections/#{collection.handle}/entries/#{entry.id}/edit",
