  ...
  module Cp
    class GlobalsController < BaseController
      def index
        ...
        render inertia: "cp/globals/Index", props: {
          sets: sets.map { |item| { handle: item.handle, title: item["title"], url: "/cp/globals/#{item.handle}/edit" } }
        }
      end
      ...
      def edit
        ...
        render inertia: "cp/globals/Edit", props: {
          title: item["title"],
          breadcrumbs: [ { label: "Globals", url: cp_globals_path }, { label: item["title"] } ],
          resource_key: "global",
          ...
          urls: { update: edit_cp_global_path(item.handle).sub("/edit", ""), publish: nil, unpublish: nil, submit: nil,
                  approve: nil, reject: nil, discard: nil, trash: nil, preview: nil, versions: nil,
                  listing: cp_globals_path, create_another: nil }
        }
      end
      ...
      def update
        item = Nibble.schema.find(:globals, params[:handle]) or raise ActiveRecord::RecordNotFound
        authorize!("globals.#{item.handle}.edit")
        result = Nibble::Lifecycle.call(global_set(item), :save, attrs, actor: Nibble::Current.user)
        return redirect_back_or_to(edit_cp_global_path(item.handle), inertia: { errors: result.errors.transform_values { |messages| Array(messages).first } }) unless result.ok?

        redirect_to edit_cp_global_path(item.handle), notice: "Saved."
      end
