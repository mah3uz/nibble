...
function submit() {
  if (editing.value) form.transform((data) => ({ redirect: data })).patch(`/admin/redirects/${editing.value.id}`)
  else form.transform((data) => ({ redirect: data })).post('/admin/redirects')
  editing.value = null
  form.reset()
}
...
async function remove(redirect: Redirect) {
  ...
  if (ok) router.delete(`/admin/redirects/${redirect.id}`)
}
</script>
