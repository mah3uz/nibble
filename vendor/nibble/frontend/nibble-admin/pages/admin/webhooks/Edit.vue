...
const base = computed(() => (props.webhook.id ? `/admin/webhooks/${props.webhook.id}` : null))
...
function save() {
  ...
  else request.post('/admin/webhooks', { preserveState: 'errors' })
}
...
<template>
  <div class="mx-auto max-w-5xl">
    ...
    >
      <AdminIcon name="chevron-left" class="size-4" />Webhooks
    </Link>
    <PageHeader
      :title="title"
      icon="webhooks"
      :breadcrumbs="[{ label: 'Webhooks', url: '/admin/webhooks' }, { label: title }]"
    >
