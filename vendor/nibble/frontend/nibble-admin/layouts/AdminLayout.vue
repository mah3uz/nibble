...
import CommandPalette from '@/components/admin/command-palette/CommandPalette.vue'
import ConfirmDialog from '@/components/admin/page/ConfirmDialog.vue'
import ElevationDialog from '@/components/admin/shell/ElevationDialog.vue'
import GlobalHeader from '@/components/admin/shell/GlobalHeader.vue'
import NavSidebar from '@/components/admin/shell/NavSidebar.vue'
import SiteSlot from '@/components/admin/SiteSlot.vue'
import ShortcutsDialog from '@/components/admin/shell/ShortcutsDialog.vue'
import { useAdmin, useFlashToasts } from '@/lib/admin'
...
useFlashToasts(() => admin.value.flash)
useTheme()
provideBreadcrumbs()
...
const stopWatchingNavigation = router.on('navigate', (event) => {
  if (event.detail.page.component.startsWith('admin/errors/') || event.detail.page.component.startsWith('admin/auth/'))
    return
  nextTick(() =>
    recordVisit(event.detail.page.url, labelForVisit(event.detail.page.url, breadcrumbs.value, admin.value.nav)),
  )
})
