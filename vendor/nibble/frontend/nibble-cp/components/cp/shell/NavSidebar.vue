...
const isActive = (active: string) => (active === '/cp' ? page.url === '/cp' : page.url.startsWith(active))
const hasActiveChild = (item: (typeof cp.value.nav)[number]['items'][number]) =>
...
<template>
  ...
  >
    <SidebarContent class="gap-6 px-3 py-6">
      <SidebarGroup v-for="section in cp.nav" :key="section.handle" class="p-0">
        <SidebarGroupLabel
          v-if="section.label"
        ...
        <SidebarGroupContent>
          <SidebarMenu class="gap-0">
            <SidebarMenuItem v-for="item in section.items" :key="item.url">
              ...
              >
                <Link :href="item.url">
                  <CpIcon :name="NAV_ICONS[item.icon] ?? item.icon" class="size-4" />
