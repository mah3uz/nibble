...
useBreadcrumbs([{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Backups' }])
</script>
...
<template>
  <div>
    <PageHeader
      title="Backups"
      icon="backups"
      :breadcrumbs="[{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Backups' }]"
    />

    <div class="mb-6 grid gap-6 md:grid-cols-2">
      <CpPanel title="Latest backup" fill>
      ...
      <CpPanel title="Off-site copies" fill>
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{
            backups.configured
