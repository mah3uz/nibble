...
function send(confirm: boolean) {
  form
    ...
    .post('/cp/utilities/content/import', { forceFormData: true, preserveState: true, preserveScroll: true })
}
...
useBreadcrumbs([{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Content' }])
</script>
...
<template>
  <div>
    <PageHeader
      title="Content"
      icon="package"
      :breadcrumbs="[{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Content' }]"
    />

    <CpPanel v-if="!can_export && !can_import">
      <p class="text-sm text-gray-600 dark:text-gray-400">
        Exporting and importing content needs its own permission, which your roles don't include.
      </p>
    </CpPanel>

    <CpPanel v-if="can_export" title="Export">
      <template #actions>
        <Button as-child variant="outline" size="sm">
          <a href="/cp/utilities/content/export" download>Download package</a>
        </Button>
      </template>
      <p class="text-sm text-gray-600 dark:text-gray-400">
    ...
    <CpPanel v-if="can_import" title="Import">
