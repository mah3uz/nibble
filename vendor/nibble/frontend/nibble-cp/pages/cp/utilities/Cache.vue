...
const clearAll = () => router.post('/cp/utilities/clear_cache', {}, { preserveScroll: true })
const purge = () => form.post('/cp/utilities/purge_cache', { preserveScroll: true, onSuccess: () => form.reset() })

useBreadcrumbs([{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Cache' }])
</script>
...
<template>
  <div>
    <PageHeader
      title="Cache"
      icon="cache"
      :breadcrumbs="[{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Cache' }]"
    >
      <template #actions><Button @click="clearAll">Clear all pages</Button></template>
    </PageHeader>

    <div class="mb-6 grid gap-6 md:grid-cols-2">
      <CpPanel title="Page cache" fill>
      ...
      <CpPanel title="Clear by tag" fill>
        <p class="text-sm text-gray-600 dark:text-gray-400">
