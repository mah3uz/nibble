...
const retry = (job: Failed) => router.post(`/cp/utilities/jobs/${job.id}/retry`, {}, { preserveScroll: true })
...
async function discard(job: Failed) {
  ...
  if (ok) router.delete(`/cp/utilities/jobs/${job.id}`, { preserveScroll: true })
}

useBreadcrumbs([{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Jobs' }])
</script>
...
<template>
  <div>
    <PageHeader
      title="Jobs"
      icon="jobs"
      :breadcrumbs="[{ label: 'Utilities', url: '/cp/utilities' }, { label: 'Jobs' }]"
    />

    <CpPanel v-if="!jobs.available">
    ...
    <template v-else>
      ...
      <CpPanel :title="`Failed (${jobs.failed.length})`" flush>
        <div class="text-sm">
      ...
      <CpPanel title="Running now" flush>
        <div class="text-sm">
      ...
      <CpPanel title="Schedule" :description="`${jobs.scheduled.count} waiting for their time`" flush>
        <div class="text-sm">
