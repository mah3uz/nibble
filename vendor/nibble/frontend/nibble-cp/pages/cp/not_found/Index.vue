...
import EmptyState from '@/components/cp/page/EmptyState.vue'
...
const suggest = (row: Missing) => router.visit(`/cp/redirects?from=${encodeURIComponent(row.path)}`)
</script>
      ...
      <tbody class="divide-y">
        <tr v-for="row in paths" :key="row.id">
          ...
          <td class="text-right">
            ...
            <Button variant="ghost" size="sm" @click="router.delete(`/cp/404s/${row.id}`)">Dismiss</Button>
          </td>
        </tr>
      </tbody>
    </table>
