...
import PasskeysPanel from '@/components/cp/account/PasskeysPanel.vue'
import TwoFactorPanel from '@/components/cp/account/TwoFactorPanel.vue'
...
function saveName() {
  nameForm.transform((data) => ({ user: data })).patch('/cp/account', { onSuccess: () => nameForm.defaults() })
}
...
const startPage = usePreference<CpPreferences['start_page']>('start_page', 'dashboard')
const afterSave = usePreference<CpPreferences['after_save']>('after_save', 'continue')
</script>
