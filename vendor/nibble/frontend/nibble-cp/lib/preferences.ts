...
type PreferencesProps = { cp: { preferences: Record<string, unknown> } }
...
export function usePreference<T>(key: string, fallback: T): Ref<T> {
  ...
  const initial = deepGet(page.props.cp.preferences, key)
...
async function save(key: string, value: unknown) {
  ...
  const response = await fetch('/cp/preferences', {
    method: 'PATCH',
